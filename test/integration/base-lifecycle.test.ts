...
const USER_REWARD_AMOUNT = parseEther("11");
...
describe("Base lifecycle integration", function () {
  it("runs ROTY mint → stake → Melting mint → stake → Amanda mint → reward claim", async function () {
    ...
    // 1. Deploy ROTY with temporary root, then set root for user.
    ...
    // 2. User whitelist mints ROTY for free.
    await roty.write.whitelistMint([[]], {
    ...
    // 4. User stakes ROTY non-custodially.
    ...
    // 6. User mints Melting through staking-gated paid mint.
    ...
    // 9. User mints Amanda through staking-gated paid mint.
    ...
    // 10. Register Amanda and stake Amanda too.
    ...
    await distributor.write.claim([ROUND_ID, USER_REWARD_AMOUNT, []], {
      account: user.account,
    });
