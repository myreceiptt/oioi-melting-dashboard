...
| `BASE_MAINNET_INDEXER_FROM_BLOCK` | Expected mainnet indexer start block | `.env.example` only | Not found in current indexer code. Needs implementation before wiring. | No | No | NOT APPROVED | Mainnet indexer/reward flow is not approved/implemented for production wiring yet. |
| `BASE_MAINNET_INDEXER_TO_BLOCK` | Expected mainnet indexer target block | `.env.example` only | Not found in current indexer code. Needs implementation before wiring. | No | No | NOT APPROVED | Current boundary worker is Sepolia-only. |
| `ETHEREUM_MAINNET_INDEXER_FROM_BLOCK` | Expected mainnet indexer start block | `.env.example` only | Not found in current indexer code. Needs implementation before wiring. | No | No | NOT APPROVED | Mainnet indexer/reward flow is not approved/implemented for production wiring yet. |
| `ETHEREUM_MAINNET_INDEXER_TO_BLOCK` | Expected mainnet indexer target block | `.env.example` only | Not found in current indexer code. Needs implementation before wiring. | No | No | NOT APPROVED | Current boundary worker is Sepolia-only. |
...
| `INDEXER_WORKER_BLOCK_SPAN` | Sepolia boundary worker tuning | `.github/workflows/boundary-worker.yml`, `lib/indexer/boundarySync.ts` | Existing Sepolia value | No for mainnet wiring | No | KEEP SEPARATE | Mainnet worker not approved. |
| `INDEXER_WORKER_COMMAND_TIMEOUT_MS` | Sepolia boundary worker tuning | `.github/workflows/boundary-worker.yml`, `lib/indexer/boundarySyncWorker.ts` | Existing Sepolia value | No for mainnet wiring | No | KEEP SEPARATE | Mainnet worker not approved. |
| `INDEXER_WORKER_LOCK_TTL_SECONDS` | Sepolia boundary worker tuning | `.github/workflows/boundary-worker.yml`, `lib/indexer/boundarySync.ts` | Existing Sepolia value | No for mainnet wiring | No | KEEP SEPARATE | Mainnet worker not approved. |
| `INDEXER_WORKER_RATE_LIMIT_DELAY_SECONDS` | Sepolia boundary worker tuning | `.github/workflows/boundary-worker.yml`, `lib/indexer/boundarySync.ts` | Existing Sepolia value | No for mainnet wiring | No | KEEP SEPARATE | Mainnet worker not approved. |
| `INDEXER_WORKER_RETRY_DELAY_SECONDS` | Sepolia boundary worker tuning | `.github/workflows/boundary-worker.yml`, `lib/indexer/boundarySync.ts` | Existing Sepolia value | No for mainnet wiring | No | KEEP SEPARATE | Mainnet worker not approved. |
...
Current mainnet indexer/reward env names are not fully implemented in the active worker/indexer code. Production reward claim must wait until mainnet indexing, reward calculation, proof generation, and claim QA are implemented/configured, run, verified, and explicitly approved.
