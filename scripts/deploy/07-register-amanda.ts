...
async function main() {
  ...
  console.log("Registering Amanda in OiOiSoftStaking...");
  console.log({
    network: networkName,
  ...
  if (!record.contracts.amanda) {
    throw new Error(`Missing Amanda deployment for ${config.key}.`);
  }
  ...
  if (alreadyApproved) {
    console.log("Amanda already approved in staking.");
  } else {
    await staking.write.setCollectionApproved([record.contracts.amanda, true]);
