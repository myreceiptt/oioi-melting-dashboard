...
async function main() {
  ...
  console.log("Registering Melting in OiOiSoftStaking...");
  console.log({
    network: networkName,
  ...
  if (alreadyApproved) {
    console.log("Melting already approved in staking.");
  } else {
    await staking.write.setCollectionApproved([record.contracts.melting, true]);
